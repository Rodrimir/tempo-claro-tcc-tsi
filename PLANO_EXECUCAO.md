...
Não altere nada fora do escopo da tarefa.
...
| **4**   | Escopo novo essencial                                                   | 7       | L       | E3         |
...
**Aceite:** `docs/MAPA_DO_CODIGO.md` existe e lista as 10 telas, os 12 endpoints e as 9 tabelas + 1 view.

> **Atualizado em 31/08/2026:** números originais (11 endpoints, 5 tabelas) refletiam o estado do
> repositório em 27/08, antes da migração para o schema v2.1 (E0.5) e de todo o trabalho de
> execução até a E4.2. `GET /api/me` (E1.5) é o endpoint novo; `sub_atividades`, `calibracoes`,
> `calibracao_respostas` e `dispositivos_push` são as tabelas novas (as duas últimas reservadas
> para D3/D4, sem uso ainda). Ver `docs/MAPA_DO_CODIGO.md` para o levantamento completo atual.
...
### ☐ E4.4 — Avatar evolutivo por nível `L`

**Especificação fechada em 31/08/2026:** ver `docs/AVATARES_ESPECIFICACAO.md`. Decisão: **5 variações visuais por personagem, cada uma cobrindo 10 níveis (50 níveis no total)**, com v5 travando a partir do dia 40 e espaço reservado para expandir depois.

```
nivel_avatar = LEAST(50, 1 + sta_dias_seguidos)
variacao     = LEAST(5, CEIL(nivel_avatar / 10.0))
```

Troca visual nos dias 10, 20, 30 e 40 — coerente com o "a cada 10 dias" que o RF14 já prometia.

**Volume de arte:** 75 combinações (3 personagens × 5 variações × 5 expressões). Duas existem; uma delas precisa redesenho. **74 a produzir.** Por isso a tarefa foi dividida — as três primeiras subtarefas são código e não dependem de nenhuma imagem nova.

---@@@@@@@@@@@@@@@@@@@@@222

#### ☐ E4.4.1 — Regra de nível e variação (backend) `S`

**▶ Prompt**

```
1. A coluna sta_nivel_avatar já existe (DEFAULT 1, CHECK >= 1). NÃO adicione
   teto no CHECK — o limite de 50 é regra de serviço, para permitir expansão
   futura sem migração
2. No FechamentoDiarioJob, ao apurar a virada do dia:
      nivel = Math.min(50, 1 + diasSeguidos)
   Recalcule junto com dias_seguidos, no mesmo ponto onde a ofensiva é atualizada
3. Ao zerar a ofensiva, o nível volta para 1 — a regressão visual é parte
   da mecânica, não um bug
4. Exponha nivel_avatar no HabitoResponseDTO
5. A VARIAÇÃO não vai para o banco — é derivada no frontend a partir do nível.
   Guardar as duas coisas seria duplicar a mesma informação
```

**Aceite:** hábito com 10 dias de ofensiva devolve `nivel_avatar: 11`; com 40 dias devolve `41`; com 500 dias devolve `50`.

---@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2

#### ☐ E4.4.2 — Resolvedor de arte com fallback (frontend) `M` ⭐

**Esta é a subtarefa que torna a entrega parcial de arte segura.** Faça-a antes de produzir qualquer imagem.

**▶ Prompt**

```
Crie um resolvedor único de avatar no frontend — nenhuma tela deve montar
caminho de imagem por conta própria.

Assinatura: resolverAvatar(categoria, nivelAvatar, expressao)

1. Derive a variação: Math.min(5, Math.ceil(nivel / 10))
2. Convenção de caminho:
   assets/avatares/{categoria}/v{variacao}/{expressao}.webp
3. Cadeia de fallback, nesta ordem exata:
   a. {categoria}/v{variacao}/{expressao}.webp
   b. {categoria}/v{variacao}/normal.webp
   c. {categoria}/v1/{expressao}.webp
   d. {categoria}/v1/normal.webp
   e. placeholder genérico
4. Use import.meta.glob do Vite para montar o manifesto em tempo de build —
   assim arte ausente é detectada na compilação, não em produção
5. Em desenvolvimento, logue no console qual nível de fallback foi usado,
   para eu saber quais artes ainda faltam sem abrir o diretório

Expressões válidas: normal, preocupado, desesperado, falha, feliz
Categorias: agua, estudo, exercicio
```

**Aceite:** com só `agua/v1/normal.webp` no disco, pedir `(agua, 45, desesperado)` devolve o arquivo de v1 sem lançar erro nem quebrar layout.

---

#### ☐ E4.4.3 — Ligar o resolvedor às telas `S`

**▶ Prompt**

```
Substitua todo uso direto de imagem de avatar por resolverAvatar():

- Home: expressão vem da urgência (proximo_vencimento, já implementado na E2.1)
  e vira "feliz" quando status == COMPLETED (E1.1)
- PreTask: expressão "normal" da variação atual
- Success: expressão "feliz"
- Fail: expressão "falha"

Remova os emojis usados como placeholder de estudo e exercício.
Confirme que nenhum componente monta caminho de asset manualmente.
```

---

#### ☐ E4.4.4 — Produção da arte _(tarefa do Rodrigo, não do Claude Code)_ `XL`

Checklist completo em `docs/AVATARES_ESPECIFICACAO.md`, seção 5. Ordem de produção:

| Onda  | Conteúdo                                  | Qtd           | Efeito                                                               |
| ----- | ----------------------------------------- | ------------- | -------------------------------------------------------------------- |
| **A** | v1 completa, 3 personagens × 5 expressões | 15 (13 novas) | Fecha F35 inteiro. Garante que o fallback nunca chega ao placeholder |
| **B** | `normal` + `feliz` de v2–v5               | 24            | Torna a evolução perceptível                                         |
| **C** | as 3 expressões restantes de v2–v5        | 36            | Completa a matriz                                                    |

> **Parada segura em qualquer onda.** Só a Onda A já entrega "o avatar reage ao prazo" de forma honesta, com a evolução por nível declarada como trabalho futuro. Onda A + B entrega as duas mecânicas. A Onda C é polimento.

> **Redesenho, não reaproveitamento:** o `gotinha/normal.png` atual mostra o personagem de braços cruzados e cara fechada — lê como brava, não neutra. Sendo o rosto que o usuário mais vê, num app cuja tese é acessibilidade cognitiva, ele contradiz a proposta.
...
| 9 — Documentação       | 4                                                      | ☐          |                          |
| **Total**              | **57** (54 no caminho de execução — Etapa 5 não entra) |            |                          |

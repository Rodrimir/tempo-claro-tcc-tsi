// @audit-ok: BACKEND-HabitoRepository.java-01
...
public class HabitoRepository {
    ...
    private static final String FIND_ALL_BY_USUARIO_ID = 
            "SELECT * FROM habitos WHERE usuario_id = ? AND ativo = true";
            
    private static final String FIND_BY_ID = 
            "SELECT * FROM habitos WHERE id = ?";
            
    private static final String INSERT_HABITO = 
            "INSERT INTO habitos (id, usuario_id, titulo, categoria, gatilho_ancora, tipo_medida, " +
    ...
    private static final String UPDATE_HABITO = 
            "UPDATE habitos SET titulo = ?, meta_base = ?, ativo = ? WHERE id = ?";
            
    private static final String ARCHIVE_HABITO = 
            "UPDATE habitos SET ativo = false WHERE id = ?";
    ...
    private final RowMapper<Habito> rowMapper = (rs, rowNum) -> Habito.builder()
            ...
            .intervaloMinutos(rs.getObject("intervalo_minutos") != null ? rs.getInt("intervalo_minutos") : null)
    ...
    public void update(Habito habito) {
        jdbcTemplate.update(UPDATE_HABITO, habito.getTitulo(), habito.getMetaBase(), habito.getAtivo(), habito.getId());
    }
